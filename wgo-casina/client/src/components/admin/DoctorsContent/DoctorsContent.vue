<template>
  <div style="width: 100%">
    <q-card v-if="!innerLoading" flat class="fit">
        ...
        <div class="row items-center justify-between q-table">
          <div class="col-12 col-sm-auto no-wrap">
            <div class="q-table__title ellipsis text-h6">
              {{ getLabel(translations.CASINA_DOCTORS_TITLE) }}
            </div>
          </div>
      ...
      <q-card-section>
        <TranslationComponent
          ...
          @onChange="onChangeDoctorsContent"
        />
      </q-card-section>
      ...
      <q-card-section>
        <Table
          :title="translations.CASINA_DOCTORS_TABLE_TITLE"
          :data="doctors"
          :schema="schema"
    ...
    <DoctorContentDialog
      ...
      @close="closeDialog"
      @success="loadDoctorsContent"
    />
  </div>
</template>

<script lang="ts" src="./DoctorsContent.ts" />
