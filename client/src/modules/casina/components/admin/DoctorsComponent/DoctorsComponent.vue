    ...
    <q-card v-if="!innerLoading" flat bordered class="bg-grey-1">
      <q-card-section>
        <div class="row items-center justify-between q-table">
          <div class="col-12 col-sm-auto no-wrap">
            <div class="q-table__title ellipsis">
              {{ translationContent.CASINA_DOCTORS_ADMIN_TITLE }}
            </div>
          </div>
        ...
        <div>
          <TranslationComponent
            :translation="transDoctor"
            :onChange="
              (langId, value) =>
                onChangeIndexContent(traslationDoctorValue, langId, value)
            "
          />
        </div>
      </q-card-section>
      ...
      <q-card-section>
        <div>
          <q-input
            ...
            :autofocus="true"
            :label="translationContent.CASINA_DOCTORS_ADMIN_SEARCH_LABEL"
      ...
      <q-card-section>
        <div class="q-pa-none" style="width: 100%">
          <q-list v-if="doctors.length">
            <template v-for="(doctor, index) in doctors">
              <DoctorEditorExpanded
                :key="`doctor${doctor.id}index${index}`"
                :doctor="doctor"
                :onSuccess="onSuccess"
                icon="badge"
                :title="doctor.content.name"
                groupName="storageDoctors"
...
<script lang="ts" src="./DoctorsComponent.ts" />
