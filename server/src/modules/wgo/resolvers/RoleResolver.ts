...
import { RoleFilterArgs, RoleInputGQL, RoleListResponseGQL, RoleResponseGQL } from '../modules';
...
export class RoleResolver {
  ...
  async addRole(@Arg('data') { name }: RoleInputGQL): Promise<Response<RolEntity>> {
    const role = new RolEntity();
    return await this._roleDataSerive.create(role);
  }

  @Mutation(() => RoleResponseGQL)
  async updateRole(@Arg('data') { id, name }: RoleInputGQL): Promise<Response<RolEntity>> {
