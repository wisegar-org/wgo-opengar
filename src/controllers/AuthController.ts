...
import { UserLoginSuccessResponse, SuccessRequest, ErrorRequest, RolEntityEnum, UserRegisterErrorResponse, UserLoginErrorResponse } from '../models/index'
...
export class AuthController {
    constructor(
        ...
        @InjectRepository(RolEntity, "development") private readonly RolRepository: RoleRepository,
        @InjectRepository(MediaEntity, "development") private readonly MediaRepository: MediaRepository,
     ...
     async loginUser(req: Request, res: Response) {
        ...
        if (_.isEmpty(userEntity) ||
            ...
            const resp = new UserLoginErrorResponse("Invalid user or invalid password")
            res.send(resp)
            return;
        }
        ...
        const JWTObj = new JwtService({ privateKey: privateKey, publicKey: publicKey });
        ...
        if (_.isUndefined(tokenResult) || _.isEmpty(tokenResult.token)) {
            return res.send(new UserLoginErrorResponse("Token generation error." + tokenResult.error))
        }
     ...
     async registerCustomer(req: Request, res: Response) {
        const { name, lastName, userName, email, password, profileImage }:
            { name: string, lastName: string, userName: string, email: string, password: string, profileImage: number } = req.body

        //checking if all parameters have a value
        if (_.isEmpty(name) || _.isEmpty(lastName) || _.isEmpty(userName) || _.isEmpty(email) || _.isEmpty(password)) {
        ...
        if (usersCount > 0) {
            res.send(new UserRegisterErrorResponse('userName', 'Error in register user: user name already exist'))
            return;
        }
        ...
        if (usersCount > 0) {
            res.send(new UserRegisterErrorResponse('email', 'Error in register user: email already exist'))
            return;
        }
        ...
        const user = new UserEntity(name, lastName, userName, email, password, [rol], false, profileImg)
        ...
        const tokenResult = JWTObj.generateToken(userObj);
        if (_.isUndefined(tokenResult) || _.isEmpty(tokenResult.token)) {
            return res.send(new ErrorResponse("Token generation error." + tokenResult.error))
        }
        ...
        EmailServer.sendEmail({
            ...
            return res.send(new ErrorResponse("Error sending email after user register"))
        });
    }
            ...
            if (!userEntity.isEmailConfirmed) {
                ...
                await this.UserRepository.save(userEntity)
                    .then(() => {
                        ...
                        return res.send(new ErrorResponse('Error trying to update isEmailConfirmed in user entity'))
                    })
                    .catch(() => {
                        return res.send(new ErrorResponse('Error trying to update isEmailConfirmed in user entity'))
                    })
     ...
     async resendEmailConfirmation(req: Request, res: Response) {
        ...
        EmailServer.sendEmail({
            ...
            return res.send(new ErrorResponse("Error sending email for resend verification account"))
        });
    }
