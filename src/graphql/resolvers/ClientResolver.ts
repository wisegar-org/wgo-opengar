...
import { RoleListResponseGQL, RoleResponseGQL } from '../types/responses/RoleResponseGQL';
...
import { ClientListResponseGQL, ClientResponseGQL } from '../types/responses/ClientResponseGQL';
import { ClientFilterArgs, ClientInputGQL } from '../types/inputs/ClientInputGQL';
...
export class ClientResolver {
    ...
    async addClient(@Arg("data")
    { name, lastName, sex, birthDate, civilStatus, principalAddress, principalCap, principalCity,
        principalMail, principalPhone, secundaryAddress, secundaryCap, secundaryCity, secundaryMail, secundaryPhone
    }: ClientInputGQL): Promise<Response<ClientEntity>>
    {
        const client = new ClientEntity(name, lastName, birthDate, sex, civilStatus)
    ...
    @Mutation(() => ClientResponseGQL)
    async updateClient(@Arg("data")
    {id, name, lastName, sex, birthDate, civilStatus, principalAddress, principalCap, principalCity,
        principalMail, principalPhone, secundaryAddress, secundaryCap, secundaryCity, secundaryMail, secundaryPhone
    }: ClientInputGQL): Promise<Response<ClientEntity>> {
        ...
        client.principalAddress = principalAddress ? principalAddress : client.principalAddress
        ...
        client.principalPhone = principalPhone ? principalPhone : client.principalPhone
        client.secundaryAddress = secundaryAddress ? secundaryAddress : client.secundaryAddress
        ...
        client.secundaryPhone = secundaryPhone ? secundaryPhone : client.secundaryPhone
