...
export class RoleDataService {
  ...
  update = async (role: RolEntity): Promise<Response<RolEntity>> => {
    ...
    if (!roleResponse.isSuccess) {
      return ErrorResponse.Response(
        `Error trying to update role','Role not found with id:${id}`
      );
    }
  ...
  remove = async (id: number): Promise<Response<RolEntity>> => {
    if (!_.isNumber(id)) {
      return ErrorResponse.Response(
        "Error trying to remove role",
        "Role id must be number"
      );
    }
